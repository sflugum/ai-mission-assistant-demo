...
![Docs Status](https://img.shields.io/badge/docs-updating-orange) ![GitHub last commit](https://img.shields.io/github/last-commit/sflugum/ai-mission-assistant-demo/main)
...
|------|-----------------|
| `backend/src/db/migrations/001_initial_schema.sql` | `public.missions` table exists (migrations applied). |
...
* **Database**: Neon (Postgres), Docker (Local)
...
3. **Environment Variables**:
   ...
   * Backend (no `VITE_` prefix): `GOOGLE_API_KEY`, `DATABASE_URL`
   * Frontend (browser): optional `VITE_API_URL`

4. **Database migrations**: Run database migrations against the same database your backend uses (for example, apply migration files in `backend/src/db/migrations` to your Neon Postgres database). Saving missions requires the `public.mission_lines` table from `20260514120000_mission_lines.sql`.
